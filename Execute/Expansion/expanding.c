...
char    *find_in_env(t_envlist *envlist, char *key)
{
    ...
    while (cur)
    {
        if (strncmp(key, cur->variable, o_ft_strlen(key)) == 0)
...
void expand_env_variables(t_tree *node, t_data *data)
{
    ...
    while (node->argv[i])
    {
        ...
        // printf(RED" was double q [ %d ]"RST"\n", node->was_d_quote);
        // printf(RED" was single q [ %d ]"RST"\n", node->was_s_quote);
        ...
        if (is_expandable(node->argv[i]) && !is_fully_single_quoted(og_string))
            node->argv[i] = expand_variable(node->argv[i], data); // here might have a leak .
